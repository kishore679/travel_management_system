...
import '../models/bucket_list_item.dart'; // Ensure this is the correct import

class StartLiveTripScreen extends StatefulWidget {
  final BucketListItem? trip; // Optional trip parameter for editing

  const StartLiveTripScreen({super.key, this.trip});

  @override
  _StartLiveTripScreenState createState() => _StartLiveTripScreenState();
}

class _StartLiveTripScreenState extends State<StartLiveTripScreen> {
  ...
  void initState() {
    ...
    // If a trip is passed, pre-fill the fields
    if (widget.trip != null) {
      ...
      // Initialize goals
      goals = List<Goal>.from(widget.trip!.goals); // Ensure the same type
    }
  }
  ...
  void _saveTrip() {
    if (destinationController.text.isNotEmpty) {
      final trip = BucketListItem(
        ...
        goals: goals, // Save the goals
        estimatedBudget: targetBudget, // Save the budget
      );

      // Instead of saving to _tripBox, we will pass the trip data back to HomeScreen
            ...
            onPressed: () {
              ...
              if (widget.trip != null) {
                Provider.of<TripProvider>(context, listen: false)
                    .deleteOngoingTrip(widget.trip!);
              }

              // Close the StartLiveTripScreen
              Navigator.pop(context); // Close the dialog
              Navigator.pop(context); // Close the StartLiveTripScreen
            },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Start Live Trip"),
